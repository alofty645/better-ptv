...
const generateSignature = (path: string): string => {
  const url = path.includes("?") ? `${path}&devid=${DEV_ID}` : `${path}?devid=${DEV_ID}`;
...
// PTV API Service
export const ptvService = {
  // Search for stops or routes
  search: async (term: string, routeTypes: number[] = [0]): Promise<{ stops: Stop[] }> => {
    try {
  ...
  getDepartures: async (stopId: number, routeType: number, maxResults = 5): Promise<DeparturesResponse> => {
    try {
